...
if (!columna_bd_existe($con, 'personas', 'activo')) {
    redirigir('php/listados/lista_preceptores.php?estado=err&msg=' . urlencode('Falta migración de producción. Ejecutá migracion_005_produccion.sql.'));
}

$id_operador = (int)($_SESSION['id_persona'] ?? 0);
if ($id_persona === $id_operador) {
    redirigir('php/listados/lista_preceptores.php?estado=err&msg=' . urlencode('No podés inactivar tu propio usuario desde este listado.'));
}
...
if (!$sentencia) {
    redirigir('php/listados/lista_preceptores.php?estado=err&msg=' . urlencode('No se pudo preparar la inactivación.'));
}
...
if ($ok && $afectadas > 0) {
    registrar_auditoria_boletin($con, [
        ...
        'payload' => ['modulo' => 'lista_preceptores']
    ]);
    redirigir('php/listados/lista_preceptores.php?estado=ok&msg=' . urlencode('Preceptor inactivado correctamente.'));
}

redirigir('php/listados/lista_preceptores.php?estado=err&msg=' . urlencode('No se pudo inactivar el preceptor.'));
